...
const Footer: React.FC = () => {
  ...
  useEffect(() => {
    registerItem('footer-global');
    registerItem('footer-comm');
  ...
  return (
    <footer className="bg-secondary pt-24 pb-12 border-t border-ui/5">
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 mb-20">
        <div className="space-y-6">
          <Link to="/" className="text-3xl font-hero text-ui tracking-tighter">
            {displayTitle}<span className="text-accent">.</span>
          </Link>
          <p className="text-ui/50 font-body text-sm leading-relaxed">
            {displayDescription}
          </p>
          <div className="flex space-x-4">
            {socialLinks.map((social, i) => (
              <a 
                ...
                className="w-10 h-10 rounded-full bg-primary flex items-center justify-center text-ui/40 hover:text-accent hover:bg-primary/80 transition-all"
              >
                <social.icon size={20} />
              </a>
            ))}
        ...
        <div>
          ...
          <ul className="space-y-4">
            {['Portfolio', 'Services', 'Equipment', 'Academy', 'Contact'].map(link => (
              <li key={link}>
                <Link to={`/${link.toLowerCase()}`} className="text-ui/40 hover:text-accent transition-colors text-sm font-subheading">
                  {link}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="space-y-4">
            <li className="flex items-start space-x-3 text-ui/40 text-sm">
              <MapPin size={18} className="text-accent shrink-0" />
              <span>{communicationData.contactAddress}</span>
            </li>
            <li className="flex items-center space-x-3 text-ui/40 text-sm">
              <Phone size={18} className="text-accent shrink-0" />
              <span>{communicationData.contactPhone}</span>
            </li>
            <li className="flex items-center space-x-3 text-ui/40 text-sm">
              <Mail size={18} className="text-accent shrink-0" />
              <span>{communicationData.contactEmail}</span>
            </li>
          </ul>
        </div>
      </div>

      <div className="max-w-7xl mx-auto px-6 pt-12 border-t border-ui/5 flex flex-col md:flex-row justify-between items-center text-ui/20 text-[10px] uppercase tracking-[0.2em]">
